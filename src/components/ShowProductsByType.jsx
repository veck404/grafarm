...
export default function ShowProductsByType({MyProducts}) {
  ...
  const MAX_PRICE = Math.max(500, ...(MyProducts || []).map(p => p?.cost || 0));
  ...
  const initTypeCheckBox = {
    ...
    price:[parseInt(queryParams.get('min'))||0,parseInt(queryParams.get('max'))||MAX_PRICE],
          ...
          <div className='flex justify-between flex-col gap-3'>
              <div className='flex justify-between cursor-pointer' onClick={()=>setArrows({type:'style'})}>
                ...
                <div>{
                  ...
                  <div className='flex flex-col justify-center flex-grow gap-6 h-fit'>
        <h1 className='ml-2 sm:ml-10 mt-5 text-2xl sm:text-3xl font-bold'>Palm Oil Products</h1>
          <motion.div
          ...
          >
            {
              Products.length?Products.map((el,index) => {
                return(
                  <motion.div key={index} variants={{ hidden: { opacity: 0, y: 16 }, visible: { opacity: 1, y: 0 } }} whileTap={{ scale:0.98 }} className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border border-neutral-200 dark:border-neutral-800 overflow-hidden">
                    <div className="flex flex-col h-full">
                      <motion.div className="bg-gray-100 dark:bg-neutral-800 cursor-pointer select-none aspect-[4/3]" onClick={handleClick(el.name)} whileHover={{scale:1.02}}>
                        <Img className="w-full h-full" draggable="false" src={el.src} alt={el.src} />
                      </motion.div>
                      <div className="p-4">
